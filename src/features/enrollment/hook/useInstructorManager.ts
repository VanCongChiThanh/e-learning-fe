...
const generateMockQuizData = (enrollmentId: UUID) => ({
    ...
    lastAttemptDate: new Date(Date.now() - Math.floor(Math.random() * 30) * 24 * 60 * 60 * 1000),
    quizDetails: Array.from({ length: Math.floor(Math.random() * 5) + 3 }, (_, index) => ({
        id: `quiz-${index}`,
...
export const useCourseStudents = (courseId: UUID | undefined) => {
    ...
    const fetchCourseStudents = useCallback(async () => {
        ...
        try {
            ...
            const enhancedEnrollments: EnrollmentWithStats[] = enrollments.map((enrollment: any) => ({
                ...enrollment,
            ...
            const completedStudents = enhancedEnrollments.filter(e => e.status === 'COMPLETED').length;
            const activeStudents = enhancedEnrollments.filter(e =>
                e.status === 'ACTIVE'
            ).length;

            const totalProgress = enhancedEnrollments.reduce((sum, e) => sum + (e.progressPercentage || 0), 0);
            const averageProgress = totalStudents > 0 ? Math.round(totalProgress / totalStudents) : 0;

            const totalWatchTime = enhancedEnrollments.reduce((sum, e) => sum + (e.totalWatchTimeMinutes || 0), 0);
    ...
    const fetchStudentStats = useCallback(async () => {
        ...
        try {
            ...
            const lectureStats = {
                ...
                completedLectures: progressList?.filter((p: any) => p.isCompleted).length || 0,
                totalWatchTime: progressList?.reduce((sum: number, p: any) => sum + (p.watchTimeMinutes || 0), 0) || 0,
                averageWatchPercentage: progressList?.length > 0
                    ? Math.round(progressList.reduce((sum: number, p: any) => sum + (p.watchedPercentage || 0), 0) / progressList.length)
                    : 0,
            };
            ...
            setStudentStats({
                ...
                totalStudyTime: lectureStats.totalWatchTime + (quizStats.quizDetails?.reduce((sum: number, q: any) => sum + q.timeSpent, 0) || 0),
            });
