...
const MyCoursesInstructor: React.FC = () => {
          ...
          for (let page = 2; page <= totalPages; page++) {
            promises.push(
              axiosAuth.get("/courses/instructor", {
                params: { order: "desc", page: page, paging: 10, sort: "created_at" },
              })
            );
          }
  ...
  const coursesForClientFilter = searchedCourses !== null ? searchedCourses : allCourses;

  const uniqueCategories = useMemo(() => {
    if (allCourses.length === 0) return ["ALL"];
    const categories = new Set(allCourses.map((c: Course) => c.category).filter(Boolean));
      ...
      <div className="container mx-auto px-6 py-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow p-6 text-center">
            <div className="text-2xl font-bold text-[#106c54]">{stats.total}</div>
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <div className="flex flex-wrap items-center gap-4">
            ...
            />
            <button onClick={handleSearch} className="bg-[#106c54] text-white px-6 py-2 rounded-lg hover:bg-[#0d5a45] transition-colors">
        ...
        {loading || isSearching ? (
          ...
          <div className="bg-white rounded-lg shadow p-12 text-center">
            ...
            <p className="text-gray-500 mb-6">Không có khóa học nào phù hợp với bộ lọc của bạn. Hãy thử lại hoặc tạo khóa học mới.</p>
            <Link
              to="/instructor/courses/create"
