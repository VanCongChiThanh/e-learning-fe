...
import { checkReviewExist, getProgressSummary } from "../api";
...
const LearningHeader: React.FC<LearningHeaderProps> = ({ title, courseId, enrollmentId, onOpenReviewModal, refreshTrigger = 0 }) => {
  ...
  useEffect(() => {
    ...
    const checkReviewStatus = async () => {
      try {
        ...
        const hasReviewed = await checkReviewExist(courseId);
        if (hasReviewed) {
          setCanReview(false);
          return;
        }
        ...
        // 2. Check progress
        const progressData = await getProgressSummary(enrollmentId);
        if (progressData && progressData.progressPercentage > 2) {
          setCanReview(true);
        } else {
          setCanReview(false);
        }
    ...
    checkReviewStatus();
  }, [courseId, enrollmentId, refreshTrigger]);
      ...
      <div className="flex items-center gap-4">
        {canReview && (
          <button 
            ...
            className="flex items-center gap-2 text-gray-700 hover:text-[#106c54] font-medium transition-colors"
          >
